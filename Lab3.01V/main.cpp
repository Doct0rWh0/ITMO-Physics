...
pair<int, int> getNearestX(const vector<Point>& field, long double x, long double y);
pair<int, int> getNearestY(const vector<Point>& field, long double x, long double y);
long double distance(long double a_x, long double a_y, long double b_x, long double b_y);

int main(int argc, char** argv){
    ifstream input;
    vector<Point> electricPotential;
    vector<Vector> electricField;
    Point dotBuffer;
    char buffer;
    Pixel* image;
    ...
    for (double y = y_start; y <= y_end; y += grid_step){
        for (double x = x_start; x <= x_end; x += grid_step){
            pair<int, int> x_dots = getNearestX(electricPotential, x, y);
            pair<int, int>  y_dots = getNearestY(electricPotential, x, y);
